...
if(_bool && alive _unit) then
{
	...
	{
		(vehicle _unit) addAction  ["Pull out wounded", {[_this, "tmf_sws_fmc_EjectWounded", true] spawn BIS_fnc_MP;}, nil, 5, false, true, "", "_target distance _this < 5 && [_target] call tmf_sws_fnc_HasWounded"];
	};
}
else
{
	...
	if(local _unit) then
	{
		...
		// reset the PP
		F_UncToggle = true;
		[] spawn tmf_sws_fnc_WoundedEffect;
		sleep 0.1;
